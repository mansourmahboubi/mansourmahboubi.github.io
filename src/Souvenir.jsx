...
function Souvenir() {
  return (
    <div style={SouvenirStyles.container}>
      <header style={SouvenirStyles.header}>
        <h1 style={SouvenirStyles.title}>Traditional Iranian Delicacies</h1>
        <p style={SouvenirStyles.subtitle}>
          Discover the authentic taste of Persian sweets and souvenirs
        </p>
      </header>

      <div style={SouvenirStyles.grid}>
        {items.map((item, index) => (
          <div
            key={index}
            style={{ ...SouvenirStyles.card, ...SouvenirStyles.cardHover }}
            onMouseOver={(e) => {
          ...
          >
            <h2 style={SouvenirStyles.itemTitle}>{item.name}</h2>
            <p style={SouvenirStyles.description}>{item.description}</p>
          </div>
        ))}
      </div>
    </div>
